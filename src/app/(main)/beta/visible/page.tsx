// src/app/(main)/beta/visible/page.tsx
...
async function getVisibleMixtapes(page: number) {
  const res = await fetch(BACKEND_URL + "/mixtapes/visible?page=" + page + "&limit=72", {
    next: { revalidate: 60 }
  })
...
export default async function VisibleMixtapesPage({
  searchParams
}: {
  ...
  const { mixtapes, pagination } = await getVisibleMixtapes(page)
  
  return (
    <div className="w-full px-4 py-8">
      <h1 className="text-2xl font-bold mb-2">Visible Mixtapes</h1>
