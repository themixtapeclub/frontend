// app/api/wantlist-v2/route.js - FIXED VERSION with secret key
import { NextResponse } from 'next/server';

const backendAPI = {
  async request(method, endpoint, data) {
    const storeId = process.env.NEXT_PUBLIC_SWELL_STORE_ID;
    const secretKey = process.env.SWELL_SECRET_KEY; // USING SECRET KEY NOW!
    ...
    if (!storeId || !secretKey) {
      ...
      throw new Error('Missing Swell credentials - need SWELL_SECRET_KEY');
    }
    ...
    console.log(`🔍 Account API Request: ${method} ${url}`);
...
// GET /api/wantlist-v2 - Get customer's wantlist from account content
export async function GET(request) {
  try {
    ...
    console.log('🔍 Getting wantlist for customer via account:', customerId);
    ...
    // Filter active items and fetch product details
    ...
    const itemsWithProducts = await Promise.all(
      activeItems.map(async (item) => {
        try {
          const product = await backendAPI.get(`/products/${item.product_id}`);
          let variant = null;
    ...
    console.error('GET /api/wantlist-v2 error:', error);
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}

// POST /api/wantlist-v2 - Add item to wantlist via account content
export async function POST(request) {
  try {
    const body = await request.json();
    const { customerId, productId, email, variantId } = body;

    console.log('🔍 POST /api/wantlist-v2 received:', { customerId, productId, email, variantId });
    ...
    console.error('POST /api/wantlist-v2 error:', error);
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}

// DELETE /api/wantlist-v2 - Remove item from wantlist via account content
export async function DELETE(request) {
  try {
    ...
    console.error('DELETE /api/wantlist-v2 error:', error);
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}
