...
async function getFilterCondition(slug: string): Promise<string> {
  // Test basic sanityClient connectivity
  try {
    const testQuery = `*[_type == "product"][0..2]{_id, title}`;
    await sanityClient.fetch(testQuery);
  } catch (error) {
    return `"${slug}" in genre[].main`;
  }
  // Check cache first
...
async function getFeaturedProducts(slug: string): Promise<any[]> {
  ...
  const featuredFilter = `${BASE_PRODUCT_FILTER} && (${filterCondition}) && week[0] != "0001" && defined(swellProductId) && stock > 0 && (inStock == true || inStock == null) && featured == true`;

  // Add missing fields for play button and complete product data
  ...
  const finalQuery = `*[${featuredFilter}] | order(coalesce(orderRank, "zzz") asc) [0...6] {
  ${OPTIMIZED_QUERY_FIELDS}
}`;

  try {
    const products = await sanityClient.fetch(finalQuery);

    if (!products || !Array.isArray(products)) return [];

    // Complete product transformation with all required fields
    const transformedProducts = products.map((product: any) => ({
      ...
      price: product.price, // Keep original value
      currency: 'USD',
      stock: product.stock,
      ...
      // Add tracklist and related fields for play button
      ...
      // Add sanityContent structure that ProductCard expects
      sanityContent: {
        _id: product._id,
        title: product.title,
        artist: product.artist,
...
// Add this test right after your getFeaturedProducts function
async function testGetProductsByArchive(slug: string) {
  try {
    const result = await getProductsByArchive('genre', slug, 1, 10, true);
  } catch (error) {}
}
...
const genreConfig = {
  ...
  getProductsFunction: async (slug: string, page: number, sort: string) => {
    await testGetProductsByArchive(slug);
    ...
    try {
      ...
      const [regularResult, featuredProducts] = await Promise.all([
        getProductsByArchive('genre', slug, page, limit, false), // <-- Change to false
